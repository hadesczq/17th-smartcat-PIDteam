...
{
    spi_mosi(IPS200_SPIN,IPS200_CS,&dat,&dat,1,1);
}
...
void ips200_wr_data16(uint16 dat)		//向液晶屏写一个16位数据
{
    ...
    IPS200_DC(1);
    spi_mosi(IPS200_SPIN,IPS200_CS,dat1,dat1,2,1);    //写入数据  高位在前  低位在后
}
...
{
    spi_init(IPS200_SPIN, IPS200_SCL, IPS200_SDA, IPS200_SDA_IN, IPS200_CS, 0, 50*1000*1000);//硬件SPI初始化
	
    gpio_init(IPS200_BL_PIN,GPO,1,PUSHPULL);
    gpio_init(IPS200_DC_PIN,GPO,0,PUSHPULL);
    gpio_init(IPS200_REST_PIN,GPO,0,PUSHPULL);
...
void ips200_displayimage032(uint16 start_x, uint16 start_y, uint16 width, uint16 height, uint8 *p)
{
    ...
    ips200_address_set(start_x, start_y, start_x + coord_x-1, start_y + coord_y-1);
